 ...
 * class Db
 *
 * @param {String} _url: CouchDB URL in format
 *
 * http(s)://user:pass@host:port/db for database
 * http(s)://user:pass@host:port/db/design/view for view
 */
...
Db.prototype.paginate = function(interval, startKey, endKey, pageSize, numPages, pageCb, cb) {
  ...
  function _action(cb) {
    ...
    // unlike startkey/startkey_docid, null endkey/endkey_docid does not mean last document
    // in the database don't set endkey_docid option unless endKey argument is set
    if (endKey) {
      opts.endkey = endKey;
    ...
    function _actionCb(err, result) {
      if (err) {
        ...
        if (result.rows.length < opts.limit || pageCount === numPages - 1) {
          ...
          interval = 0;
          // if not last page, then use the last document's key/id as the startkey/startkey_docid
          // of the first document on the next page
        } else {
...
Db.prototype.update = function(docs, cb) {
  ...
  // nano bulk callback is called with null error and unmodified documents in CouchDB as the result,
  // ignoring the modified documents. Ideally nano should pass an error.
